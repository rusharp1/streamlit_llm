...
def get_dictionary_chain():
    ...
    prompt = ChatPromptTemplate.from_template(
        ...
        초과 금액은 단순히 사용자가 제시한 금액에서 기준 금액을 뺀 값으로 계산해주세요.
...
def get_history_retriever():
    ...
    few_shot_prompt = FewShotChatMessagePromptTemplate(
        ...
        # "당신은 소득세법 전문가입니다. 사용자의 소득세법에 대한 질문에 답변해주세요.\n"
        # "아래에 제공된 문서를 활용해서 답변해 주시고\n"
        # "답변을 알 수 없다면 모른다고 답변해 주세요\n"
        # "답변을 제공할 때는 소득세법 (xx조)에 따르면 이라고 시작하면서 답변해 주세요\n"
    ...
    contextualize_q_prompt = ChatPromptTemplate.from_messages(
        [
            ("system", contextualize_q_system_prompt), # LLM 의 역할.
            # Few shot 를 아래에 넣어 history 로 추가
            # 처음 채팅을 시작하면 history가 없는데, few_shot_prompt가 그 역할을 해줄 수 있음.
            few_shot_prompt,
